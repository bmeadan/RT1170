...
#if defined(XIP_BOOT_HEADER_ENABLE_INSIDE) && XIP_BOOT_HEADER_ENABLE_INSIDE
...
status_t flexspi_erase_image(uint32_t addr) {
    status_t status;
    int sector = 0;
    while (sector < SECTOR_COUNT) {
        status = flexspi_nor_flash_erase_sector(FLASH_FLEXSPI, addr);
