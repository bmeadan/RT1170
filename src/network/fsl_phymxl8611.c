...
/*! @brief Defines the PHY MXL8611 vendor defined registers. */
...
/*! @brief Defines the PHY MXL8611 ID number. */
#define PHY_CONTROL_ID1 (0xC133U) /*!< The PHY ID1 . */

/*! @brief Defines the mask flag in interrupt enable register. */
#define PHY_INER_LINKSTATUS_CHANGE_MASK  ((1U << 10) | (1U << 11)) // LSTC | LFST
...
/*! @brief Defines the PHY MXL8611 extra page and the registers in specified page. */
...
#define PHY_MXL8611_WRITE(handle, regAddr, data) \
    ((phy_mxl8611_resource_t *)(handle)->resource)->write((handle)->phyAddr, regAddr, data)
#define PHY_MXL8611_READ(handle, regAddr, pData) \
    ((phy_mxl8611_resource_t *)(handle)->resource)->read((handle)->phyAddr, regAddr, pData)

/*******************************************************************************
 * Prototypes
 ******************************************************************************/

static status_t PHY_MXL8611_MMD_SetDevice(phy_handle_t *handle,
                                          uint8_t device,
                                          uint16_t addr,
                                          phy_mmd_access_mode_t mode);
static inline status_t PHY_MXL8611_MMD_ReadData(phy_handle_t *handle, uint16_t *pData);
static inline status_t PHY_MXL8611_MMD_WriteData(phy_handle_t *handle, uint16_t data);
static status_t PHY_MXL8611_MMD_Read(phy_handle_t *handle, uint8_t device, uint16_t addr, uint16_t *pData);
static status_t PHY_MXL8611_MMD_Write(phy_handle_t *handle, uint8_t device, uint16_t addr, uint16_t data);

/*******************************************************************************
 * Variables
 ******************************************************************************/

const phy_operations_t phymxl8611_ops = {.phyInit             = PHY_MXL8611_Init,
                                         .phyWrite            = PHY_MXL8611_Write,
                                         .phyRead             = PHY_MXL8611_Read,
                                         .getAutoNegoStatus   = PHY_MXL8611_GetAutoNegotiationStatus,
                                         .getLinkStatus       = PHY_MXL8611_GetLinkStatus,
                                         .getLinkSpeedDuplex  = PHY_MXL8611_GetLinkSpeedDuplex,
                                         .setLinkSpeedDuplex  = PHY_MXL8611_SetLinkSpeedDuplex,
                                         .enableLoopback      = PHY_MXL8611_EnableLoopback,
                                         .enableLinkInterrupt = PHY_MXL8611_EnableLinkInterrupt,
                                         .clearInterrupt      = PHY_MXL8611_ClearInterrupt};
...
status_t PHY_MXL8611_Init(phy_handle_t *handle, const phy_config_t *config) {
    ...
    /* Check PHY ID. */
    do {
        result = PHY_MXL8611_READ(handle, PHY_ID1_REG, &regValue);
    ...
    /* Reset PHY. */
    result = PHY_MXL8611_WRITE(handle, PHY_BASICCONTROL_REG, PHY_BCTL_RESET_MASK);
    if (result != kStatus_Success) {
        return result;
    }

    do {
        result = PHY_MXL8611_READ(handle, PHY_BASICCONTROL_REG, &regValue);
        if (result != kStatus_Success) {
            return result;
        }
    ...
    // result = PHY_MXL8611_WRITE(handle, PHY_PAGE_SELECT_REG, PHY_PAGE_RGMII_TXRX_DELAY_ADDR);
    // if (result != kStatus_Success)
    // {
    //     return result;
    // }
    // /* Set Tx Delay. */
    // result = PHY_MXL8611_READ(handle, PHY_RGMII_TX_DELAY_REG, &regValue);
    // if (result == kStatus_Success)
    // {
    //     regValue |= PHY_RGMII_TX_DELAY_MASK;
    //     result = PHY_MXL8611_WRITE(handle, PHY_RGMII_TX_DELAY_REG, regValue);
    //     if (result != kStatus_Success)
    //     {
    //         return result;
    //     }
    ...
    // /* Set Rx Delay. */
    // result = PHY_MXL8611_READ(handle, PHY_RGMII_RX_DELAY_REG, &regValue);
    // if (result == kStatus_Success)
    // {
    //     regValue |= PHY_RGMII_RX_DELAY_MASK;
    //     result = PHY_MXL8611_WRITE(handle, PHY_RGMII_RX_DELAY_REG, regValue);
    //     if (result != kStatus_Success)
    //     {
    //         return result;
    //     }
    ...
    // result = PHY_MXL8611_WRITE(handle, PHY_PAGE_SELECT_REG, 0x0);
    // if (result != kStatus_Success)
    // {
    //     return result;
    // }
    ...
    // {
    //     /* Get capabilities */
    //     result = PHY_MXL8611_MMD_Read(handle, PHY_MDIO_MMD_PCS, PHY_MDIO_PCS_EEE_CAP, &regValue);
    //     if (result == kStatus_Success)
    //     {
    //         /* Enable EEE for 100TX and 1000T */
    //         result = PHY_MXL8611_MMD_Write(handle, PHY_MDIO_MMD_AN, PHY_MDIO_AN_EEE_ADV,
    //                                         regValue & (PHY_MDIO_EEE_1000T | PHY_MDIO_EEE_100TX));
    //     }
    // }
    // else
    // {
    //     result = PHY_MXL8611_MMD_Write(handle, PHY_MDIO_MMD_AN, PHY_MDIO_AN_EEE_ADV, 0);
    // }
    // if (result != kStatus_Success)
    // {
    //     return result;
    ...
    result = PHY_MXL8611_WRITE(handle, PHY_PAGE_SELECT_REG, PHY_PAGE_INTR_PIN_ADDR);
    if (result != kStatus_Success) {
        return result;
    }
    result = PHY_MXL8611_READ(handle, PHY_PAGE_INTR_PIN_REG, &regValue);
    if (result == kStatus_Success) {
        regValue &= ~PHY_PAGE_INTR_PIN_MASK;
        result = PHY_MXL8611_WRITE(handle, PHY_PAGE_INTR_PIN_REG, regValue);
        if (result != kStatus_Success) {
            return result;
        }
    } else {
    ...
    /* Restore to default page 0 */
    result = PHY_MXL8611_WRITE(handle, PHY_PAGE_SELECT_REG, 0);
    if (result != kStatus_Success) {
        return result;
    }

    result = PHY_MXL8611_ClearInterrupt(handle);
    if (result != kStatus_Success) {
        return result;
    }
    ...
    if (config->autoNeg) {
        ...
        result = PHY_MXL8611_WRITE(handle, PHY_AUTONEG_ADVERTISE_REG,
        ...
        if (result == kStatus_Success) {
            result = PHY_MXL8611_WRITE(handle, PHY_1000BASET_CONTROL_REG, PHY_1000BASET_FULLDUPLEX_MASK);
            if (result == kStatus_Success) {
                result = PHY_MXL8611_READ(handle, PHY_BASICCONTROL_REG, &regValue);
                // if (result == kStatus_Success)
                // {
                //     result = PHY_MXL8611_WRITE(handle, PHY_BASICCONTROL_REG,
        ...
        result = PHY_MXL8611_WRITE(handle, PHY_BASICCONTROL_REG, regValue);
        if (result != kStatus_Success) {
            return result;
        }

        /* Disable the auto-negotiation and set user-defined speed/duplex configuration. */
        result = PHY_MXL8611_SetLinkSpeedDuplex(handle, config->speed, config->duplex);
    }
    if (result != kStatus_Success) {
        return result;
    }
    result = PHY_MXL8611_EnableLinkInterrupt(handle, config->intrType);

    return result;
}

status_t PHY_MXL8611_Write(phy_handle_t *handle, uint8_t phyReg, uint16_t data) {
    return PHY_MXL8611_WRITE(handle, phyReg, data);
}

status_t PHY_MXL8611_Read(phy_handle_t *handle, uint8_t phyReg, uint16_t *pData) {
    return PHY_MXL8611_READ(handle, phyReg, pData);
}
...
status_t PHY_MXL8611_GetAutoNegotiationStatus(phy_handle_t *handle, bool *status) {
    ...
    *status = false;
    ...
    result = PHY_MXL8611_READ(handle, PHY_BASICSTATUS_REG, &regValue);
    if (result == kStatus_Success) {
...
status_t PHY_MXL8611_GetLinkStatus(phy_handle_t *handle, bool *status) {
    ...
    result = PHY_MXL8611_READ(handle, PHY_SPECIFIC_STATUS_REG, &regValue);
    if (result == kStatus_Success) {
...
status_t PHY_MXL8611_EnableLoopback(phy_handle_t *handle, phy_loop_t mode, phy_speed_t speed, bool enable) {
    ...
    if (enable) {
        ...
        if (result == kStatus_Success) {
            ...
            result = PHY_MXL8611_WRITE(handle, PHY_BASICCONTROL_REG, (regValue | PHY_BCTL_RESTART_AUTONEG_MASK));
        }
    }
    return result;
}
...
status_t PHY_MXL8611_EnableLinkInterrupt(phy_handle_t *handle, phy_interrupt_type_t type) {
    ...
    result = PHY_MXL8611_READ(handle, PHY_INER_REG, &regValue);
    if (result != kStatus_Success) {
        return result;
    }
    ...
    result = PHY_MXL8611_WRITE(handle, PHY_INER_REG, regValue);
    if (result != kStatus_Success) {
        return result;
    }
...
status_t PHY_MXL8611_ClearInterrupt(phy_handle_t *handle) {
    ...
    return PHY_MXL8611_READ(handle, PHY_INSR_REG, &regValue);
}
...
static status_t PHY_MXL8611_MMD_SetDevice(phy_handle_t *handle,
    ...
    result = PHY_MXL8611_WRITE(handle, PHY_MMD_ACCESS_CONTROL_REG, device);
    if (result != kStatus_Success) {
        return result;
    }

    /* Set register address. */
    result = PHY_MXL8611_WRITE(handle, PHY_MMD_ACCESS_DATA_REG, addr);
    if (result != kStatus_Success) {
        return result;
    }

    /* Set Function mode of data access(b01~11) and device address. */
    result = PHY_MXL8611_WRITE(handle, PHY_MMD_ACCESS_CONTROL_REG, (uint16_t)mode | (uint16_t)device);
    return result;
}

static inline status_t PHY_MXL8611_MMD_ReadData(phy_handle_t *handle, uint16_t *pData) {
    return PHY_MXL8611_READ(handle, PHY_MMD_ACCESS_DATA_REG, pData);
}

static inline status_t PHY_MXL8611_MMD_WriteData(phy_handle_t *handle, uint16_t data) {
    return PHY_MXL8611_WRITE(handle, PHY_MMD_ACCESS_DATA_REG, data);
}

static status_t PHY_MXL8611_MMD_Read(phy_handle_t *handle, uint8_t device, uint16_t addr, uint16_t *pData) {
    status_t result = kStatus_Success;
    result          = PHY_MXL8611_MMD_SetDevice(handle, device, addr, kPHY_MMDAccessNoPostIncrement);
    if (result == kStatus_Success) {
        result = PHY_MXL8611_MMD_ReadData(handle, pData);
    }
    return result;
}
...
static status_t PHY_MXL8611_MMD_Write(phy_handle_t *handle, uint8_t device, uint16_t addr, uint16_t data) {
    ...
    result = PHY_MXL8611_MMD_SetDevice(handle, device, addr, kPHY_MMDAccessNoPostIncrement);
    if (result == kStatus_Success) {
        result = PHY_MXL8611_MMD_WriteData(handle, data);
    }
    return result;
}
